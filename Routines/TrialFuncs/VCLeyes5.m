%VLC CORRELATOR SIMPLE SIMULATION FOR FACE RECOGNITION-using only eyes as ref 5-LUIS CARLOS DURAN
%Date: 08-05-2020
...
%HERE THE ZEROS ARE ADDED TO THE IMAGES MATRICES
...
%so that their sizes coincide with the target's size. NOTE that these i,j values work for all references 
%since they do have the same size (the references)

%Padarray a number around matrix, only works for image toolbox.
Ir= padarray(Ir,[i j],0);Ir2= padarray(Ir2,[i j],0); Ir3= padarray(Ir3,[i j],0); Ir4= padarray(Ir4,[i j],0); Ir5= padarray(Ir5,[i j],0); 
Ir6= padarray(Ir6,[i j],0); Ir7= padarray(Ir7,[i j],0); Ir8= padarray(Ir8,[i j],0); Ir9= padarray(Ir9,[i j],0); Ir10= padarray(Ir10,[i j],0); 
