%VLC CORRELATOR SIMPLE SIMULATION FOR FACE RECOGNITION-LUIS CARLOS DURAN
%Date: 03-05-2020
%This code calculates and shows the result of the correlation function
%between a target and a sigle reference image, using the method of a VLC type architecture.
%A PHASE ONLY filter is used.
clear all;
close all;
clf;
clc;
%Images have 3 DIM-; rgb images have 2 dim.
addpath('C:\Users\Lucho xD\Desktop')
%REFERENCE
img_r=imread('face1.png');
%TARGETS
img_t=imread('face1.png');img_t=imresize3(img_t,size(img_r));% Resizes the image
img_t2=imread('face2.png');img_t2=imresize3(img_t2,size(img_r));
