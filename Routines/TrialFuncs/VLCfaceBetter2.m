...
%This codes includes all 3
%normalization methods, which can be exchanged by commenting (with %)
%certain parts properly, this also works if one wants to run the code
%without normalizing.
clear all;
close all;
clc;
...
%TEST IMAGE
img_t=imread('Sample219.png');img_t=imresize3(img_t,size(img_r{1}));img_t=im2double(img_t);%img_t=normalize(img_t);
...
%TARGET IMAGES
It= rgb2gray(img_t);%It=im2double(It);
It=normalize(It);
