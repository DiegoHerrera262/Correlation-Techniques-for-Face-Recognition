%VLC CORRELATOR SIMPLE SIMULATION FOR FACE RECOGNITION IMPROVED 2-LUIS CARLOS DURAN
%Date: 23-05-2020
...
%then use the adjustment function with values [0.07 0.2] and finally perform a new normalization. 
clear all;
close all;
clc;
clf;
...
%img_r{k}=im2double(img_r{k}); %Make image a double
%img_r{k}=normalize(img_r{k}); %Normalize image 
end
%TEST IMAGE
img_t=imread('Sample218.png');img_t=imresize3(img_t,size(img_r{1}));%img_t=im2double(img_t);%img_t=normalize(img_t);
...
%TARGET IMAGES
It= rgb2gray(img_t); It=im2double(It);It=normalize(It);
