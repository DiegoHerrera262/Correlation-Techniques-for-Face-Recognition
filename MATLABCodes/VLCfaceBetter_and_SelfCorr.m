%VLC CORRELATOR SIMPLE SIMULATION FOR FACE RECOGNITION IMPROVED 5(SELF CORRELATION CALCULATION) -LUIS CARLOS DURAN
%Date: 30-05-2020
...
%coeficients for the linear combination are computed based on a PCE criteria. The code also calculates self-correlation.
...
%TEST IMAGE
picture=read_files(10).name %This is to watch the name of the image used at the moment
...
picture=read_files(10).name %This is to watch the name of the image used at the moment
P2=Ft.*sign(real(A(10).*Fr{10})); %A(k) is the coefficient for the kth image of the set
...
figure(3);imshow(It);title('Test image (Edge detection + Label function)');
