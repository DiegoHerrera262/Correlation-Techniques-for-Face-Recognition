...
%between a target and a reference image, using the method of a VLC type architecture and multiple references(50).
...
%then use the adjustment function with values [0.07 0.2], perform a new normalization. Then use the imcomplement function and
%finally apply the edge detection function with the log standard method.
clear all;
close all;
clc;
clf;
...
%TEST IMAGE
img_t=imread('Sample236.png');img_t=imresize3(img_t,size(img_r{1}));%img_t=im2double(img_t);%img_t=normalize(img_t);
...
It= rgb2gray(img_t); 
%It=edge(It)
It=im2double(It);It=normalize(It);
It=imadjust(It,[0.07 0.2]); %[low_in high_in]
It=normalize(It);It=imcomplement(It);
It=edge(It,'log');        % edge(I,method,threshold,sigma) especifica , la desviación estándar del filtro.sigma 
                    %Esta sintaxis solo es válida cuando es o .method'log''Canny'
